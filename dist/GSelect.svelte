...
let { class: classList = '', hideLabel = false, hint = '', id = '', label, options = [], required = false, value = $bindable(null), ...restProps } = $props();
let uid = $derived(id || uniqueId('u'));
let formattedOptions = $derived(options.map((option) => {
    if (!option)
